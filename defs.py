...
def sigma(dist,std_wind=None,turb=None,stab= None,tables=False):
    ...
    '''
    if stab == None:
        stab = stability_class(std_wind,turb,v=True)
...
def OTA33A(gasConc,temp,pres,ws2,wd2,ws3z,ws3x,ws3y,wslimit,wdlimit,cutoff,distance,theta_start,theta_end,delta_theta,mw_chemical,chemical_name,make_plot=False):
    '''
    ...
    ws3x, ws3y, ws3z, wd3, ws3 = sonic_correction(ws3x,ws3y,ws3z)
    ...
    gasConc -= np.mean(gasConc[np.where(gasConc < np.percentile(gasConc[:],5))]) # [g m-3]
    ...
    for i in range(indices.min(),indices.max()):
        temp_vals = gasConc[:][np.where(indices == i)]
    ...
    fit_amplitude = ppm2gm3(fit_gasConc[0],mw_chemical,np.mean(temp[~wd3_mask]),np.mean(pres[~wd3_mask]))
    ...
    massRate = 2*np.pi*fit_amplitude*np.mean(ws3[~wd3_mask])*sy*sz # [g s-1]
    ...
    # now L/min at STP
    volumeRate = 2*np.pi*fit_amplitude*np.mean(ws3)*sy*sz/rho_gasConc_stp*60 # [L min-1]
